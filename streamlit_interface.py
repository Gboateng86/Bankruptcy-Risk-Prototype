...
import pandas as pd
import matplotlib
matplotlib.use("Agg")   # no display window needed on a server; saves memory
...
plt.tight_layout()
st.pyplot(fig_imp)
plt.close(fig_imp)   # free the figure's memory
...
if st.button("Predict Bankruptcy Risk", type="primary"):
    ...
    plt.tight_layout()
    st.pyplot(fig_op)
    plt.close(fig_op)   # free the figure's memory
